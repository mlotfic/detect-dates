...
>>> dm.validate_date('gregorian', 29, 2, 1900)
False
"""
...
class DateMapping:
    """
    Date mapping and conversion between Gregorian, Hijri, and Solar Hijri calendars.

    Loads a mapping DataFrame and provides methods for calendar conversion, weekday lookup,
    month/year queries, and date validation.
    ...
    def __init__(self):
        """
        Initialize DateMapping by loading the mapping DataFrame.
    ...
    def get_date_alternative_calendar(
        self, era: str, day: int, month: int, year: int
    ) -> Optional[Dict[str, Union[Dict[str, int], str]]]:
        """
    ...
    def get_dates_by_month_year(
        self, era: str, month: int, year: int
    ) -> List[Dict[str, Union[Dict[str, int], str]]]:
        """
    ...
    def get_date_range(
        self, era: str, start_date: Dict[str, int], end_date: Dict[str, int]
    ) -> List[Dict[str, Union[Dict[str, int], str]]]:
        """
        ...
        --------
        >>> dm = DateMapping()
        >>> dm.get_date_range('gregorian', {'day': 1, 'month': 5, 'year': 1900}, {'day': 5, 'month': 5, 'year': 1900})
        [{'gregorian': {...}, ...}, ...]
        """
        era_columns = {
            'gregorian': ['Gregorian Day', 'Gregorian Month', 'Gregorian Year'],
        ...
        # Helper: convert date to comparable integer for range filtering
        ...
        # Create comparable column for filtering
