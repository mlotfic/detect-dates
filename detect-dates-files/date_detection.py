...
# Import from normalizers
from normalizers import (
    # Month normalization and keywords
    normalize_month,  # Main month normalization function
    normalize_month_to_num,  # Convert month names to numbers
    months_keywords,  # All month keywords
    
    # Month names in different calendars and languages
    months_gregorian_en,
    months_gregorian_ar, 
    months_hijri_en,
    months_hijri_ar,
    ...
    weekdays_en,  # English weekday names
    weekdays_ar,  # Arabic weekday names
    weekdays_fa_ar,  # Persian-Arabic weekday names
    weekdays_fa_en,  # Persian-English weekday names

    # Date indicators and separators
    indicators_keywords,  # Date component indicators
    numeric_words_ar,  # Arabic numeric words
    numeric_words_pattern_ar,  # Regex pattern for Arabic numeric words
)

# Import from calendar_variants
from calendar_variants import (
...
LANG = "ar"  # or "en", etc.
# lang = LANG
patterns = get_date_patterns(LANG)

# =======================================
# Access patterns directly for regex usage
# =======================================
# --- Date Indicators (like "يوم", "شهر", "عام") ---
day_indicator_pattern       = patterns.day_indicator_pattern
month_indicator_pattern     = patterns.month_indicator_pattern
year_indicator_pattern      = patterns.year_indicator_pattern
century_indicator_pattern   = patterns.century_indicator_pattern

# --- Connectors and Ranges (like "من", "إلى") ---
separator_pattern           = patterns.separator_pattern
range_connector_pattern     = patterns.range_connector_pattern
range_starter_pattern       = patterns.range_starter_pattern
...
year_num_pattern = r"(\d{1,4})"
month_num_pattern = r"(\d{1,2})"
day_num_pattern = r"(\d{1,2})"
century_num_pattern= r"(\d{1,2})"


m_yr_num_pattern = rf"{month_num_pattern}\s*{separator_pattern}?\s*{year_num_pattern}"
assert re.compile(m_yr_num_pattern, flags=re.IGNORECASE | re.UNICODE)
d_m_num_pattern = rf"{day_num_pattern}\s*{separator_pattern}?\s*{month_num_pattern}"
assert re.compile(d_m_num_pattern, flags=re.IGNORECASE | re.UNICODE)
d_m_yr_num_pattern = rf"{day_num_pattern}\s*{separator_pattern}?\s*{month_num_pattern}\s*{separator_pattern}?\s*{year_num_pattern}"
assert re.compile(d_m_yr_num_pattern, flags=re.IGNORECASE | re.UNICODE)
...
assert re.compile(hijri_y_pattern_s, flags=re.IGNORECASE | re.UNICODE)

gregorian_y_pattern = rf"{year_num_pattern}\s*{separator_pattern}?\s*{gregorian_era_pattern}"
assert re.compile(gregorian_y_pattern, flags=re.IGNORECASE | re.UNICODE)
...
natural_hijri_d_m_y_pattern_num = rf"{day_pattern}\s*{separator_pattern}?\s*{d_m_yr_num_pattern}\s*{separator_pattern}?\s*{hijri_era_pattern}"
...
natural_hijri_d_m_y_pattern_name = rf"{day_pattern}\s*{separator_pattern}?\s*{day_num_pattern}\s*{separator_pattern}?\s*{hijri_month_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern_s}"
...
assert re.compile(century_pattern_s, flags=re.IGNORECASE | re.UNICODE)
...
PURE_DATE_PATTERNS = [
    {  # Pattern 0 - Month/Year Numeric (Ambiguous Calendar)
        "pattern": m_yr_num_pattern,
        "name": "ambiguous_numeric_month_year",  # Original: "month_year_num_pattern"
        "description": "Numeric month/year format - ambiguous calendar detection required",
        ...
        "date": {
            ...
            "year": 2,       # Second capture group
            "century": None, 
            "era": None, 
            "calendar": "Numeric"  # Default assumption - should be overridden by context
        },
    },
    
    {  # Pattern 1 - Day/Month/Year Numeric (Ambiguous Calendar)
        "pattern": d_m_yr_num_pattern,
        "name": "ambiguous_numeric_full_date",  # Original: "day_month_year_num_pattern"
        ...
        "date": {
            ...
            "century": None, 
            "era": None, 
            "calendar": "Numeric"  # Default - needs contextual override
        },
    },
    
    {  # Pattern 2 - Day/Month/Year with Weekday Prefix
        "pattern": rf"{day_pattern}\s*{separator_pattern}?\s*{d_m_yr_num_pattern}",
        ...
        "date": {
            "weekday": 1,    # First capture group (weekday name)
            "day": 2,        # Second capture group
            "month": 3,      # Third capture group
            "year": 4,       # Fourth capture group
            "century": None, 
            "era": None, 
            "calendar": "Numeric"  # Default - weekday can help validate
        },
    },
    
    # REMOVED: Pattern 3 was identical to Pattern 1 - duplicate removed to prevent conflicts
    # Original Pattern 3 had hardcoded values (day=1, month=12, year=1440) which was incorrect
    
    {  # Pattern 4 - Basic Hijri Year with Era Marker
        "pattern": hijri_y_pattern,
    ...
    {  # Pattern 7 - Hijri Month/Year with Era Marker
        ...
        "date": {
            "weekday": None, 
            "day": None, 
            "month": [1, 4],   # First or fourth capture group (month)
    ...
    {  # Pattern 10 - Complete Hijri Date with Era Marker
        ...
        "date": {
            "weekday": None, 
            "day": [1, 5],     # First or fifth capture group (day)
            "month": [2, 6],   # Second or sixth capture group (month)
    ...
    {  # Pattern 11 - Complete Gregorian Date with Era Marker
        "pattern": gregorian_d_m_y_pattern,
    ...
    {  # Pattern 13 - Natural Language Hijri Date
        ...
        "date": {
            "weekday": [1, 6],   # First or sixth capture group (weekday name)
            "day": [2, 7],       # Second or seventh capture group (day)
            "month": [3, 8],     # Third or eighth capture group (month name)
    ...
    {  # Pattern 14 - Natural Language Gregorian Date
        "pattern": natural_gregorian_pattern,
        ...
        "examples": [  # Original: "example": "الجمعة 15 يناير 2023 ميلاديًا"
...
# ===================================================================================
# MIXED NUMERIC DATE PATTERNS - UNKNOWN ERA
# ===================================================================================
MIXED_PURE_DATE_PATTERNS_UNKNOWN_ERA = [
    {  # Pattern 0 - Numeric month/year combo (ambiguous calendar)
        "pattern": rf"{m_yr_num_pattern}\s*{separator_pattern}?\s*{m_yr_num_pattern}",  # TODO: Define base patterns
        "name": "ambiguous_month_year_numeric",  # More descriptive
        "description": "Matches ambiguous month/year in numeric format without era indicators",
    ...
    {
        "pattern": rf"{d_m_yr_num_pattern}\s*{separator_pattern}?\s*{d_m_yr_num_pattern}",
        "name": "ambiguous_full_date_numeric",  # More descriptive
        "description": "Matches ambiguous day/month/year in numeric format without era indicators",
        "examples": [  # Enhanced examples
            "15/03/1445",
            "02/01/2023",
            "01/12/1440",
    ...
    {
        "pattern": rf"{day_pattern}\s*{separator_pattern}?\s*{d_m_yr_num_pattern}\s*{separator_pattern}?\s*{day_pattern}\s*{separator_pattern}?\s*{d_m_yr_num_pattern}",  # TODO: Define base patterns
        "name": "ambiguous_weekday_date_numeric",  # More descriptive
        "description": "Matches ambiguous weekday + day/month/year in numeric format",
        "examples": [  # Enhanced examples
            "الثلاثاء 02/01/2023",
            "الأربعاء 03/03/1398",
            "الخميس 15-07-1444",
...
MIXED_PURE_DATE_PATTERNS_SLASH = [
    {  # Pattern 0 - Hijri Year Range (start+end)
        "pattern": rf'{hijri_y_pattern_s}\s*{separator_pattern}?\s*{hijri_y_pattern}',
    ...
    {  # Pattern 1 - Gregorian Year Range
        "pattern": rf'{gregorian_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_y_pattern}',
    ...
    {  # Pattern 2 - Hijri Year Range (non-strict)
        "pattern": rf'{hijri_y_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern}',
    ...
    {  # Pattern 3 - Gregorian Year Range (non-strict)
        "pattern": rf'{gregorian_y_pattern}\s*{separator_pattern}?\s*{gregorian_y_pattern}',
    ...
    {  # Pattern 4 - Hijri/Gregorian Combined (Hijri First)
        "pattern": rf"{hijri_y_pattern}\s*{separator_pattern}?\s*{gregorian_y_pattern}",
    ...
    {  # Pattern 5 - Gregorian/Hijri Combined (Gregorian First)
        "pattern": rf"{gregorian_y_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern}",
    ...
    {  # Pattern 6 - Hijri Month-Year to Hijri Month-Year
        "pattern": rf'{hijri_m_y_pattern_s}\s*{separator_pattern}?\s*{hijri_m_y_pattern}',
    ...
    {  # Pattern 7 - Gregorian Month-Year to Gregorian Month-Year
        "pattern": rf'{gregorian_m_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}',
    ...
    {  # Pattern 8 - Hijri Month-Year Range (loose format)
        "pattern": rf'{hijri_m_y_pattern}\s*{separator_pattern}?\s*{hijri_m_y_pattern}',
    ...
    {  # Pattern 19 - Gregorian Month-Year Range (loose)
        "pattern": rf'{gregorian_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}',
    ...
    {  # Pattern 7 - Hijri + Gregorian M/Y combo (Hijri first)
        "pattern": rf"{hijri_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}",
    ...
    {  # Pattern 8 - Gregorian + Hijri M/Y combo (Gregorian first)
        "pattern": rf"{gregorian_m_y_pattern}\s*{separator_pattern}?\s*{hijri_m_y_pattern}",
    ...
    {  # Pattern 10 - Hijri full date + Gregorian full date
        "pattern": rf"{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}",
    ...
    {  # Pattern 11 - Gregorian full date + Hijri full date
        "pattern": rf"{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}",
    ...
    {   # Pattern 13 - Natural Language Hijri followed by Gregorian
        "pattern": rf"{natural_hijri_pattern}\s*{separator_pattern}?\s*{natural_gregorian_pattern}",
    ...
    {   # Pattern 14 - Natural Language Gregorian followed by Hijri
        "pattern": rf"{natural_gregorian_pattern}\s*{separator_pattern}?\s*{natural_hijri_pattern}",
    ...
    {   # Pattern 42 - Hijri Range (Day-Month-Year to Day-Month-Year)
        "pattern": rf'{hijri_d_m_y_pattern_s}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}',
    ...
    {   # Pattern 43 - Gregorian Range (Day-Month-Year to Day-Month-Year)
        "pattern": rf'{gregorian_d_m_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}',
    ...
    {   # Pattern 30 - Hijri range using same structure on both sides (legacy duplicate of Pattern 42)
        "pattern": rf'{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}',
    ...
    {   # Pattern 40 - Gregorian range duplicate (legacy - same as pattern 43)
        "pattern": rf'{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}',
    ...
    {   # Pattern 10 - Full Hijri date followed by Gregorian equivalent (shared day)
        "pattern": rf"{day_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}",
    ...
    {   # Pattern 11 - Full Gregorian date followed by Hijri equivalent
        "pattern": rf"{day_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}",
    ...
    {  # Pattern 54 - Hijri-to-Hijri date range with weekday
        "pattern": rf'{natural_hijri_pattern_s}\s*{separator_pattern}?\s*{natural_hijri_pattern}',
    ...
    {   # Pattern 55 - Gregorian-to-Gregorian date range with weekday
        "pattern": rf'{natural_gregorian_pattern_s}\s*{separator_pattern}?\s*{natural_gregorian_pattern}',
    ...
    {  # Pattern 51 - DUPLICATE of Pattern 54 — consider merging or deleting
        "pattern": rf'{natural_hijri_pattern}\s*{separator_pattern}?\s*{natural_hijri_pattern}',
    ...
    {   # Pattern 52 - Gregorian date range with weekdays and Arabic connector
        "pattern": rf'{natural_gregorian_pattern}\s*{separator_pattern}?\s*{natural_gregorian_pattern}',
...
MIXED_PURE_DATE_PATTERNS_PARENTHETICAL = [ 
   {    # Pattern 9 - Hijri Year to Hijri Year (with parentheses)
        "pattern": rf'{hijri_y_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 10 - Gregorian Year to Gregorian Year (with parentheses)
        "pattern": rf'{gregorian_y_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 6 - Hijri Year Range (basic, not start-anchored)
        "pattern": rf'{hijri_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 7 - Gregorian Year Range (basic)
        "pattern": rf'{gregorian_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 3 - Hijri year followed by Gregorian year
        "pattern": rf'{hijri_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 5 - Gregorian year followed by Hijri year
        "pattern": rf'{gregorian_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 21 - Hijri month/year to Hijri month/year range (with parentheses)
        "pattern": rf'{hijri_m_y_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_m_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 22 - Gregorian month/year to Gregorian month/year (with parentheses)
        "pattern": rf'{gregorian_m_y_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 18 - Hijri month/year to Hijri month/year (no starter anchor)
        "pattern": rf'{hijri_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 19 - Gregorian month/year to Gregorian month/year (simplified)
        "pattern": rf'{gregorian_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_m_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 7 - Hijri + Gregorian month/year combination
        "pattern": rf'{hijri_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 8 - Gregorian + Hijri month/year combination
        "pattern": rf'{gregorian_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_m_y_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 10 - Full Hijri/Gregorian Date (Hijri first)
        "pattern": rf'{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
        "name": "full_date_hijri_then_gregorian",  # Original: "hijri_full_date"
    ...
    {  # Pattern 11 - Full Gregorian/Hijri Date (Gregorian first)
        "pattern": rf'{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
        "name": "full_date_gregorian_then_hijri",  # Original: "gregorian_full_date"
    ...
    {  # 13 — Natural language Hijri date with Gregorian
        "pattern": rf'{natural_hijri_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{natural_gregorian_pattern}\s*(?:[\)\]])',
    ...
    {   # 14 — Natural Gregorian with Hijri
        "pattern": rf'{natural_gregorian_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{natural_hijri_pattern}\s*(?:[\)\]])',
    ...
    {  # 42 — Hijri to Hijri full date range
        "pattern": rf'{hijri_d_m_y_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # 43 — Gregorian to Gregorian full date range
        "pattern": rf'{gregorian_d_m_y_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # 30 — Hijri to Hijri full date range (non-start version)
        "pattern": rf'{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # 40 — Gregorian to Gregorian full date range (non-start)
        "pattern": rf'{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # 10 — Hijri full date followed by Gregorian in brackets
        "pattern": rf'{day_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {  # 11 — Gregorian full date followed by Hijri in brackets
        "pattern": rf'{day_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # 54 — Natural Hijri weekday + date range
        "pattern": rf'{natural_hijri_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{natural_hijri_pattern}\s*(?:[\)\]])',
    ...
    {   # 55 — Natural Gregorian weekday + date range
        "pattern": rf'{natural_gregorian_pattern_s}\s*{separator_pattern}?\s*(?:[\(\[])\s*{natural_gregorian_pattern}\s*(?:[\)\]])',
    ...
    {  # Pattern 52 - Gregorian Date Range with Weekdays
        "pattern": rf'{natural_gregorian_pattern}\s*{separator_pattern}?\s*(?:[\(\[])\s*{natural_gregorian_pattern}\s*(?:[\)\]])',
...
MIXED_RANGE_PATTERNS_parenthetical = [    
    {   # Pattern 9 - Hijri year range with surrounding parentheses and Gregorian shadow
        "pattern": rf'(?:[\(\[])\s*{hijri_y_pattern_s}\s*{separator_pattern}?\s*{hijri_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 10 - Gregorian year range with surrounding parentheses and Hijri shadow
        "pattern": rf'(?:[\(\[])\s*{gregorian_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_y_pattern_s}\s*{separator_pattern}?\s*{hijri_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 6 - Hijri range only, no suffix, with Gregorian range next (non-suffixed)
        "pattern": rf'(?:[\(\[])\s*{hijri_y_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_y_pattern}\s*{separator_pattern}?\s*{gregorian_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 7 - Gregorian range without suffix, followed by Hijri range without suffix
        "pattern": rf'(?:[\(\[])\s*{gregorian_y_pattern}\s*{separator_pattern}?\s*{gregorian_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_y_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 3 - Single Hijri+Gregorian pair in brackets
        "pattern": rf'(?:[\(\[])\s*{hijri_y_pattern}\s*{separator_pattern}?\s*{gregorian_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_y_pattern}\s*{separator_pattern}?\s*{gregorian_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 5 - Single Gregorian+Hijri pair in brackets
        "pattern": rf'(?:[\(\[])\s*{gregorian_y_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_y_pattern}\s*{separator_pattern}?\s*{hijri_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 21 - Hijri to Hijri range with Gregorian reference
        "pattern": rf'(?:[\(\[])\s*{hijri_m_y_pattern_s}\s*{separator_pattern}?\s*{hijri_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_m_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 22 - Gregorian to Gregorian range with Hijri reference
        "pattern": rf'(?:[\(\[])\s*{gregorian_m_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_m_y_pattern_s}\s*{separator_pattern}?\s*{hijri_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 18 - Hijri to Hijri range with Gregorian in same brackets (compact)
        "pattern": rf'(?:[\(\[])\s*{hijri_m_y_pattern}\s*{separator_pattern}?\s*{hijri_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 19 - Gregorian to Gregorian range followed by Hijri
        "pattern": rf'(?:[\(\[])\s*{gregorian_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_m_y_pattern}\s*{separator_pattern}?\s*{hijri_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 7 - Mixed Hijri/Gregorian Month-Year Format
        "pattern": rf'(?:[\(\[])\s*{hijri_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 8 - Gregorian/Hijri Cross Match Bracketed
        "pattern": rf'(?:[\(\[])\s*{gregorian_m_y_pattern}\s*{separator_pattern}?\s*{hijri_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_m_y_pattern}\s*{separator_pattern}?\s*{hijri_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 10 - Hijri/Gregorian Pair in (Hijri/Gregorian) (Hijri/Gregorian) Format
        "pattern": rf'(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 11 - Gregorian/Hijri Pair in (Gregorian/Hijri) (Gregorian/Hijri) Format
        "pattern": rf'(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 13 - Natural Hijri-Gregorian Pair in Brackets with Day Name   
        "pattern": rf'(?:[\(\[])\s*{natural_hijri_pattern}\s*{separator_pattern}?\s*{natural_gregorian_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{natural_hijri_pattern}\s*{separator_pattern}?\s*{natural_gregorian_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 14 - Natural Gregorian-Hijri Pair in Brackets with Day Name
        "pattern": rf'(?:[\(\[])\s*{natural_gregorian_pattern}\s*{separator_pattern}?\s*{natural_hijri_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{natural_gregorian_pattern}\s*{separator_pattern}?\s*{natural_hijri_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 42 - Hijri to Hijri range with symbolic and full pattern
        "pattern": rf'(?:[\(\[])\s*{hijri_d_m_y_pattern_s}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 43 - Gregorian to Gregorian range with symbolic suffixes
        "pattern": rf'(?:[\(\[])\s*{gregorian_d_m_y_pattern_s}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_d_m_y_pattern_s}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 30 - Duplicate of Pattern 42 (Consider deprecating)
        "pattern": rf'(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 40 - Duplicate of Pattern 43 (Consider merging)
        "pattern": rf'(?:[\(\[])\s*{gregorian_d_m_y_pattern}\s*{separator_pattern}?\s*{gregorian_d_m_y_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{hijri_d_m_y_pattern}\s*{separator_pattern}?\s*{hijri_d_m_y_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 54 - Hijri range with weekdays
        "pattern": rf'(?:[\(\[])\s*{natural_hijri_pattern_s}\s*{separator_pattern}?\s*{natural_hijri_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{natural_gregorian_pattern_s}\s*{separator_pattern}?\s*{natural_gregorian_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 55 - Gregorian range with weekdays
        "pattern": rf'(?:[\(\[])\s*{natural_gregorian_pattern_s}\s*{separator_pattern}?\s*{natural_gregorian_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{natural_hijri_pattern_s}\s*{separator_pattern}?\s*{natural_hijri_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 51 - Hijri range with natural weekday format
        "pattern": rf'(?:[\(\[])\s*{natural_hijri_pattern}\s*{separator_pattern}?\s*{natural_hijri_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{natural_gregorian_pattern}\s*{separator_pattern}?\s*{natural_gregorian_pattern}\s*(?:[\)\]])',
    ...
    {   # Pattern 52 - Gregorian range with natural weekday
        "pattern": rf'(?:[\(\[])\s*{natural_gregorian_pattern}\s*{separator_pattern}?\s*{natural_gregorian_pattern}\s*(?:[\)\]])\s*(?:[\(\[])\s*{natural_hijri_pattern}\s*{separator_pattern}?\s*{natural_hijri_pattern}\s*(?:[\)\]])',
...
# ===================================================================================
# NATURAL LANGUAGE HYBRID PATTERNS (TO BE REFACTORED)
# ===================================================================================
NATURAL_LANGUAGE_DATE_PATTERNS = []  # TODO: Add all natural language patterns here
...
FULL_DATE_RANGE_PATTERNS = [
    {  # Pattern 1 - Hijri Year Range
        "pattern": rf'\b{range_starter_pattern}?\s+{hijri_y_pattern}\s+{range_connector_pattern}\s+{hijri_y_pattern}\b',
    ...
    {  # Pattern 2 - Gregorian Year Range
        "pattern": rf'\b{range_starter_pattern}?\s+{gregorian_y_pattern}\s+{range_connector_pattern}\s+{gregorian_y_pattern}\b',
    ...
    {  # Pattern 4 - Hijri Year Range (alt pattern with suffix)
        "pattern": rf'\b{range_starter_pattern}?\s+{hijri_y_pattern_s}\s+{range_connector_pattern}\s+{hijri_y_pattern}\b',
    ...
    {  # Pattern 5 - Gregorian Year Range (alt pattern with suffix)
        "pattern": rf'\b{range_starter_pattern}?\s+{gregorian_y_pattern_s}\s+{range_connector_pattern}\s+{gregorian_y_pattern}\b',
    ...
    {  # Pattern 7 - Hijri Month-Year Range
        "pattern": rf'\b{range_starter_pattern}?\s+{hijri_m_y_pattern}\s+{range_connector_pattern}\s+{hijri_m_y_pattern}\b',
    ...
    {  # Pattern 8 - Gregorian Month-Year Range
        "pattern": rf'\b{range_starter_pattern}?\s+{gregorian_m_y_pattern}\s+{range_connector_pattern}\s+{gregorian_m_y_pattern}\b',
    ...
    {  # Pattern 10 - Hijri Month-Year Range (alt suffix on start)
        "pattern": rf'\b{range_starter_pattern}?\s+{hijri_m_y_pattern_s}\s+{range_connector_pattern}\s+{hijri_m_y_pattern}\b',
    ...
    {  # Pattern 11 - Gregorian Month-Year Range (alt suffix on start)
        "pattern": rf'\b{range_starter_pattern}?\s+{gregorian_m_y_pattern_s}\s+{range_connector_pattern}\s+{gregorian_m_y_pattern}\b',
    ...
    {  # Pattern 13 - Hijri Full Date Range
        "pattern": rf'\b{range_starter_pattern}?\s+{hijri_d_m_y_pattern}\s+{range_connector_pattern}\s+{hijri_d_m_y_pattern}\b',
    ...
    {  # Pattern 14 - Gregorian Full Date Range
        "pattern": rf'\b{range_starter_pattern}?\s+{gregorian_d_m_y_pattern}\s+{range_connector_pattern}\s+{gregorian_d_m_y_pattern}\b',
    ...
    {  # Pattern 16 - Hijri Full Date Range (start with suffix)
        "pattern": rf'\b{range_starter_pattern}?\s+{hijri_d_m_y_pattern_s}\s+{range_connector_pattern}\s+{hijri_d_m_y_pattern}\b',
    ...
    {  # Pattern 17 - Gregorian Full Date Range (start with suffix)
        "pattern": rf'\b{range_starter_pattern}?\s+{gregorian_d_m_y_pattern_s}\s+{range_connector_pattern}\s+{gregorian_d_m_y_pattern}\b',
    ...
    {  # Pattern 20 - Hijri Weekday-Date Range
        "pattern": rf'\b{range_starter_pattern}?\s+{natural_hijri_pattern}\s+{range_connector_pattern}\s+{natural_hijri_pattern}\b',
    ...
    {  # Pattern 21 - Gregorian Weekday-Date Range
        "pattern": rf'\b{range_starter_pattern}?\s+{natural_gregorian_pattern}\s+{range_connector_pattern}\s+{natural_gregorian_pattern}\b',
    ...
    {  # Pattern 23 - Hijri Weekday-Date Range (start with suffix)
        "pattern": rf'\b{range_starter_pattern}?\s+{natural_hijri_pattern_s}\s+{range_connector_pattern}\s+{natural_hijri_pattern}\b',
    ...
    {  # Pattern 24 - Gregorian Weekday-Date Range (start with suffix)
        "pattern": rf'\b{range_starter_pattern}?\s+{natural_gregorian_pattern_s}\s+{range_connector_pattern}\s+{natural_gregorian_pattern}\b',
