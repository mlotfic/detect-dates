...
SUPPORTED_CALENDARS = {
    ...
    'Jalali': ['Solar Hijri Day', 'Solar Hijri Month', 'Solar Hijri Year']  # Note: 'Jalali' maps to Solar Hijri
}
...
class DateMapping:
    ...
    def get_calendar(self, era: str) -> str:
        ...
        Args:
            era (str): Era string (e.g., 'AD', 'AH', 'CE', 'BCE')

        Returns:
            str: Normalized calendar name ('gregorian', 'hijri', 'Jalali')

        Raises:
            ValueError: If the era string is not recognized
        """
        era = normalize_era(era)  # Normalize
        if era:
     ...
     def validate_input_date(self, era: str, day: int, month: Union[int, str], year: int) -> Tuple[int, int, int]:
        """
        Validate the input date components for correctness.

        Args:
            day (int): Day of the month (1-31)
            month (int or str): Month of the year (1-12 or month name)
            year (int): Year in the specified calendar system

        Raises:
            ValueError: If any component is out of valid range or format
        """        
    ...
    def get_weekday_by_date(self, era: str = "AD", day: int = 1, month: Union[int, str] = 1, year: int = 2009) -> Optional[str]:
        ...
        Args:
            calendar (str): Calendar system ('gregorian', 'hijri', 'Jalali', or aliases) or era ('AD', 'AH', etc.)
    ...
    def get_date_alternative_calendar(self, calendar: str, day: int, month: int, year: int) -> Optional[Dict[str, Any]]:
        ...
        Args:
            calendar (str): Source calendar system ('gregorian', 'hijri', 'Jalali', or aliases)
    ...
    def validate_date(self, calendar: str, day: int, month: int, year: int) -> bool:
        ...
        Args:
            calendar (str): Calendar system ('gregorian', 'hijri', 'Jalali', or aliases)
