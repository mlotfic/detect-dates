...
class RegexParsedDate:
    ...
    def get_parsed_dates(self) -> ParsedDate:
        ...
        weekday = get_match_value(self.match, self.weekday) if self.weekday else None
        # Extract and convert day to integer with safe error handling
        day = get_match_value(self.match, self.day) if self.day else None
            
        # Extract month value from match object
        month = get_match_value(self.match, self.month) if self.month else None
            
        # Extract year value from match object  
        year = get_match_value(self.match, self.year) if self.year else None
            
        # Extract century value from match object
        century = get_match_value(self.match, self.century) if self.century else None
            
        # Extract era value from match object
        era = get_match_value(self.match, self.era) if self.era else None,
        
        return ParsedDate(text, weekday, day, month, year, century, era, self.calendar)
